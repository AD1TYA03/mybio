    ...
    >
      ...
      <div className={`w-4/12 ${isLeft ? 'pr-6' : 'pl-6'}`}>
        <div className="bg-white rounded-xl border border-gray-100 p-6 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col h-full backdrop-blur-sm bg-white/90">
          ...
          {/* Description */}
          <div className="mb-4">
            <p className={`text-gray-700 text-sm leading-relaxed ${!isExpanded ? 'line-clamp-3' : ''} overflow-hidden transition-all duration-300`}>
              {project.description}
            </p>
            {project.description.length > 150 && (
              <button
                onClick={() => setIsExpanded(!isExpanded)}
                className="flex items-center gap-1 text-orange-500 hover:text-orange-600 text-sm font-semibold mt-2 transition-colors"
              >
                {isExpanded ? (
                  <>
                    <ChevronUp size={14} />
